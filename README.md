...
`vicos-oelinux-1.6-rebuild` *is* the repo you are looking for if you want to create a full OTA. It includes stuff like victor as submodules.
...
```
git clone --recurse-submodules --shallow-submodules https://github.com/Switch-modder/vicos-oelinux-1.6-rebuild --depth=1
```
...
This will create an OTA called vicos-1.6.1.6080.ota in vicos-oelinux/_build/. 
This is scriptable for dev and oskr builds but not prod because a password will be needed afterwards in order to inject /anki - for dev and oskr it will not prompt for anything.
